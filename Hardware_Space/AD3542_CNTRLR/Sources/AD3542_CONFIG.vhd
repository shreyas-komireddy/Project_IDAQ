...
entity AD3542_CONFIG is
   Port ( 
          ...
          dac_rst        : out std_logic;                           -- DAC reset pin
          ...
          -- WRT_REGB: Write INTERFACE_CONFIG_B (mode register)
           when WRT_REGB =>
            ...
            if spi_done = '1' then
               ...
               dta_in <= x"000018" & "00" & config_dta(13 downto 12) & "00" & config_dta(9 downto 8); 
            end if;
          ...
          -- WRT_VREF: Internal reference configuration
           when WRT_VREF =>
            ...
            if spi_done = '1' and spi_en /= '0' then
               ...
               dta_in <= x"000019" & ctrlr_reg(5 downto 2) & ctrlr_reg(5 downto 2);
          ...
          -- WRT_REGA: Sequential writes for multi-byte register programming
           when WRT_REGA =>
            ...
            if spi_done = '1' and spi_en /= '0' then
               ...
               dta_in <= x"00000E06";                                            -- Stream Length
            end if;
         ...
         -- WRT_BYTELNGTH: Number of Bytes defined for a frame 
           when WRT_BYTELNGTH =>
            ...
            if spi_done = '1' and spi_en /= '0' then
               ...
               dta_in <= x"00000F04";                                            -- Byte Length Value Set Bit                                    
            end if;
         ...
         -- WRT_BYTEVAL: Defining the sate of previous Byte Length value
          when WRT_BYTEVAL =>
           ...
           if spi_done = '1' and spi_en /= '0' then
              ...
              dta_in <= x"00000108";                                             -- Streaming Mode
           end if;
