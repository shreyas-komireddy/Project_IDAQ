...
begin
  ...
  cs          <= cs_tmp;
  sclk        <= sclk_tmp when current_state = TOGGLE else '1';
...
--          when "00" =>                                         ----- DEFAULT MODE (1 MUPS Update Freq & ~16 MHz Serial Clock Freq) -----
--            tgl_lngth <= to_unsigned(2,2);
--            cnt_pos   <= to_unsigned(9,5); 
--            bit_pos   <= to_unsigned(3,4);                     
--            frm_lngth <= to_unsigned(130,10); 
...
--          when "10" =>                                          ----- FAST MODE (2 MUPS Update Freq & ~16 MHz Serial Clock Freq) -----
    ...
    if falling_edge(clk_in) then
       if current_state = TOGGLE then
          if ((fst_bit = '1') and (pos_cntr = bit_pos)) or ((fst_bit = '0') and (pos_cntr = (bit_pos - 2))) then
             sdi <= dta_in(to_integer(bit_cnt));
             if rd_sdo = '1' then   
                spi_outA(to_integer(bit_cnt)) <= sdo_a;
                spi_outB(to_integer(bit_cnt)) <= sdo_b;
                spi_outC(to_integer(bit_cnt)) <= sdo_c;
                spi_outD(to_integer(bit_cnt)) <= sdo_d;
             end if;
          end if;      
       else
          sdi <= '0';
