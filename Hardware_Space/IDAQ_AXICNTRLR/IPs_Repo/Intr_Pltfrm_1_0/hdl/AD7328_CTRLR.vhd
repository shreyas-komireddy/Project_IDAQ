...
component AD7328_ACQ is
  PORT ( 
         ...
         acq_sdo      : in  std_logic_vector(1 downto 0);      -- Only (4 downto 3) are decoded to determine the AD7328_ACQ logic
     ...
     if sys_rst = '0' then
        ...
        case ctrlr_state is
         ...
         -- DONE: AXI-Controlled Mode Selection  
          when DONE =>
            if adc_ctrlr(7 downto 6)    = "01" then        ---- Continous Acquisition ----
               ctrlr_state <= ACQUIRE;
            elsif adc_ctrlr(7 downto 6) = "10" then        ---- Re-configuration ----
               ctrlr_state <= CONFIG;
            elsif adc_ctrlr(7 downto 6) = "11" then        ---- Hold-
               ctrlr_state <= DONE;
            end if;
